        ...
        for(pn in pns){
          try({
            name <- paste("Istim-", as.character(r_s), 
                          ...
                          "_summary.csv", sep="")
            di <- read_csv(paste(path, name, sep=""))
...
```{r fig1_load, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}
...
mi_exp1 <- load.all.data(exp1path, "MI", rates, gs, Ns, pns)
# mi_exp1 <- rename(mi_exp1, model=X1)

# h_exp1 <- load.data(exp1path, "H", rates, gs, Ns, pns)
# h_exp1 <- rename(h_exp1, c(X1="model"))
# 
# pac_exp1 <- load.data(exp1path, "PAC", rates, gs, Ns, pns)
# pac_exp1 <- rename(pac_exp1, c(X1="model"))
```

# Fig 1 - Redraft
```{r fig1, fig.width=1.4, fig.height=1.2, echo=FALSE, warning=FALSE, message=FALSE}
print(unique(mi_exp1$Istim))
print(unique(mi_exp1$g))

mi_exp1 %>% 
  group_by(Istim, g, N, pn, trial) %>% 
...
tmp %>% 
  filter(model %in% c("add_p", "div_sub_p", "mult_p", "sub_p")) %>% 
  ggplot(aes(x=model, y=mi, color=Istim)) +
...
```{r fig1, fig.width=1, fig.height=1.2, echo=FALSE, warning=FALSE, message=FALSE}
tmp %>% 
  filter(model %in% c("add_p", "div_sub_p", "mult_p", "sub_p")) %>% 
  group_by(model) %>% 
  summarise(m=median(mi), sd=mad(mi)) -> tmp2

limits <- aes(ymax = m + sd, ymin=m - sd)

tmp2 %>% 
  ggplot(aes(x=model, y=m)) +
  geom_bar(stat="identity", width=.75, fill="black") +
...
```{r fig1, fig.width=2.4, fig.height=0.9, echo=FALSE, warning=FALSE, message=FALSE}
tmp %>% 
  filter(model %in% c("add_p", "div_sub_p", "mult_p", "sub_p")) %>% 
  ggplot(aes(x=Istim, y=mi, color=g, group=g)) +
  geom_line(alpha=0.9) +
  facet_grid(.~model) +
...
# Preprocess D
# Sel N and p
mi_fig1 <- filter(mi_exp1, N == 100, pn == 1.00)

# Get Avg MI for stim
Mstim <- mean((filter(mi_fig1, model == "stim_p"))$mean)
mi_fig1$mean <- mi_fig1$mean - Mstim

# Select, rename, and order 'em
mi_fig1 <- filter(mi_fig1, model %in% c("mult_p", "sub_p", "add_p", "div_sub_p"))
# mi_fig1$model <- revalue(mi_fig1$model, c("gain_p"="EI", "summed_p"="E", "silenced_p"="I"))
# mi_fig1$model <- factor(mi_fig1$model, levels=c("EI", "E", "I")) 

# Make summary stats
mi_stats <- group_by(mi_fig1, model)
...
# Preprocess E
mi_exp1 %>% 
  filter(N == 100, pn == 1.00, model %in% c("gain_p", "silenced_p", "summed_p", "stim_p")) %>%
  droplevels() -> 
  mi_fig1b
...
mi_fig1b %>% select(mean, Istim, g, model) %>% 
  group_by(Istim, g, model) %>%
  cast(Istim + g ~ model, value = "mean") %>%
...
# --
# Preprocess A
pac_fig2 <- filter(pac_exp1, N == 100, pn == 1.00)
...
exp1path  <- paste(path, "/exp1/", sep="")
mi_exp1a  <- load.all.data(exp1path, "MI", rates, gs, Ns, pns)
pac_exp1a <- load.all.data(exp1path, "PAC", rates, gs, Ns, pns)
h_exp1a   <- load.all.data(exp1path, "H", rates, gs, Ns, pns)

mi_exp1a <- filter(mi_exp1a, N == 100, pn == 1.00)
pac_exp1a <- filter(pac_exp1a, N == 100, pn == 1.00)
h_exp1a <- filter(h_exp1a, N == 100, pn == 1.00)

mi_exp1a <- droplevels(mi_exp1a)
pac_exp1a <- droplevels(pac_exp1a)
h_exp1a <- droplevels(h_exp1a)
```

```{r fig2b, fig.width=7.5, fig.height=4.5, echo=FALSE, warning=FALSE, message=FALSE}
# --
# Preprocess B
tmpdf <- data.frame(mi=(mi_exp1a$gain_p - mi_exp1a$stim_p), 
                    pac=(pac_exp1a$gain_p - pac_exp1a$stim_p))
...
# --
# Preprocess B2
tmpdf <- data.frame(mi=(mi_exp1a$summed_p - mi_exp1a$stim_p), 
                    pac=(pac_exp1a$summed_p - pac_exp1a$stim_p))
...
# --
# Preprocess B3
tmpdf <- data.frame(mi=(mi_exp1a$silenced_p - mi_exp1a$stim_p), 
                    pac=(pac_exp1a$silenced_p - pac_exp1a$stim_p))
