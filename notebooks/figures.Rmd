...
input = 1:100
  ...
  theme_classic() + 
  geom_curve(aes(x = 45, xend = 25, y = 55, yend = 64),
...
examples %>% 
  ...
  geom_line() +
  geom_line(aes(x=times, y=stim_ref), color="darkseagreen4") +
...
```
## Build Figure
```{r, fig.width=2.2, fig.height=2.4, echo=FALSE, warning=FALSE, message=FALSE}
b1 <- plot_spacer() + p4 + plot_layout(widths = c(0.2, 0.8))
b2 <- plot_spacer() + p5 + plot_layout(widths = c(0.2, 0.8))
b3 <- p1 + 
  ggtitle("FI modulation") + 
  theme(plot.title = element_text(size = 7, hjust=1, face = "bold")) + 
  p6 + plot_layout(widths = c(0.2, 0.8))
b4 <- p2 + p7 + plot_layout(widths = c(0.2, 0.8))
b5 <- p3 + p8 + plot_layout(widths = c(0.2, 0.8))

b1 / b2 / b3 / b4 / b5 
ggsave("figures/example.pdf", dpi=600)
```
# Figure - Entropy and probability
```{r, fig.width=3, fig.height=0.8, echo=FALSE, warning=FALSE, message=FALSE}
examples <- read_csv("../data/examples.csv")
...
example_dists %>% 
  ...
  geom_bar(fill="darkseagreen4", alpha=0.6, stat="identity") +
...
p_h <- as_ggplot(
  text_grob(TeX("\\sum_m \\frac{p log p}{log m} = \\frac{H_m}{H_{max}}"), vjust = 0.7)) +
  ggtitle("Estimate entropy") + 
  theme(plot.title = element_text(size = 7, hjust=0.9, vjust = 3, face = "bold")) 

p1 + p2 + p_h + plot_layout(widths = c(0.4, 0.2, 0.25)) 
...
example_dists %>% 
  ...
  geom_bar(aes(x=m, y=stim_ref), fill="darkseagreen4", alpha=0.6, stat="identity") +
  scale_x_continuous(breaks=0:9, labels=1:10) +
...
p1_label <- text_grob("EI", hjust=1, face="bold")
p2_label <- text_grob("E", hjust=1, face="bold")
p3_label <- text_grob("I", hjust=1, face="bold")
...
# Figure - Mutual information and oscillation
## Panel - Overall 
...
```{r, fig.width=2, fig.height=4.1, echo=FALSE, warning=FALSE, message=FALSE}
# E, I, IE
h_exp8 %>% 
  filter(q == "0.05") %>% 
  filter(num_pop == 50) %>% 
...
## Panel - FI for II
```{r, fig.width=0.8, fig.height=0.5, echo=FALSE, warning=FALSE, message=FALSE}
phi1 <- function(x) {
  30 / (1 + exp(-.2*x + 5))
}
...
input = 1:100
  ...
  geom_line(data=fi, mapping=aes(x=input, y=ref), color="grey") +
  theme_classic() + 
  labs(x="Input", y="Output", title="II modulation") +
...
## Panel - Example
```{r, fig.width=2.2, fig.height=0.6, echo=FALSE, warning=FALSE, message=FALSE}
examples <- read_csv("../data/examples.csv")
example_dists <- read_csv("../data/example_dists.csv")
...
examples %>% 
  ...
  geom_line(aes(x=times, y=stim_ref), color="darkseagreen4") +
  scale_y_continuous(breaks=c(0, 1), limits=c(0, 1)) +
  labs(x="Time", y="II", title="Timecourse") +
  theme_classic2() + 
  theme(plot.title = element_text(size = 7, hjust=1, face = "bold")) +
...
## Panel - MI
```{r, fig.width=3, fig.height=1.9, echo=FALSE, warning=FALSE, message=FALSE}
dmi_exp13 %>%
  filter(num_pop == 50) %>% 
  gather(model, mi, back_p:div_sub_p) %>% 
...
tmp %>% 
  ...
  lims(y=c(-.35, 0.5)) +
  labs(y=expression(paste(Delta, "MI")), x="", title="Overall") +
  theme_classic() + 
...
## Build Figure
```{r, fig.width=3.2, fig.height=1.7, echo=FALSE, warning=FALSE, message=FALSE}
