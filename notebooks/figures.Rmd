...
mi_exp16 <- load.data7(exp_path, "MI", stim_rates, qs, num_pops, osc_rates, qs)
dmi_exp16 <- load.data7(exp_path, "dMI", stim_rates, qs, num_pops, osc_rates, qs)
h_exp16 <- load.data7(exp_path, "H", stim_rates, qs, num_pops, osc_rates, qs)
pac_exp16 <- load.data7(exp_path, "PAC", stim_rates, qs, num_pops, osc_rates, qs)
pow_exp16 <- load.data7(exp_path, "power", stim_rates, qs, num_pops, osc_rates, qs)
...
tmp %>% 
  ...
  geom_jitter(data=p_tmp, aes(x=num_pop, y=mi), size=0.1, alpha=0.02, width=.05, color="black") +
...
```{r, fig.width=3.6, fig.height=2.3, echo=FALSE, warning=FALSE, message=FALSE}
dmi_exp18 %>% 
  filter(q == "0.5") %>% 
  gather(model, mi, back_p:div_sub_p) %>% 
  filter(model %in% c("add_p", "mult_p", "sub_p", "div_sub_p")) -> 
...
```{r, fig.width=3.5, fig.height=2.3, echo=FALSE, warning=FALSE, message=FALSE}
dmi_exp18 %>% 
  filter(q == "0.5") %>% 
  gather(model, mi, back_p:div_sub_p) %>% 
  filter(model %in% c("add_p", "mult_p", "sub_p", "div_sub_p")) -> 
...
```{r, fig.width=3.6, fig.height=1.4, echo=FALSE, warning=FALSE, message=FALSE}
# --------------------------------------------------------------
## Panel - MI
dmi_exp13 %>%
  filter(num_pop == 50) %>% 
...
dmi_exp13 %>%
  ...
  p_tmp
p_tmp$model <- revalue(p_tmp$model, c("div_sub_p"="II")) 
# p_tmp$model <- factor(p_tmp$model, levels=c("E", "EI", "I", "II"))
...
tmp %>% 
  ...
  labs(y=expression(paste(Delta, "MI")), 
       x="Stimulus firing rate", title = "Divisive fraction (II)") +
  theme_classic() +
  theme(strip.background = element_blank()) +
  theme(plot.title = element_text(size = 9, face="bold")) +
  theme(legend.background = element_rect(colour ="black")) +
