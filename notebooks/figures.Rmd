...
examples %>% 
  ...
  geom_line() +
  geom_line(aes(x=times, y=stim_ref), color="red") +
...
# Labels
p1_label <- as_ggplot(text_grob(TeX("II"), hjust=3))
...
## Build Figure
```{r, fig.width=3.1, fig.height=1.4, echo=FALSE, warning=FALSE, message=FALSE}
(p1 + plot_spacer()) / (p2 + p3) +
  plot_layout(widths = c(0.75, 10)) +
  plot_annotation(tag_levels = 'a', tag_suffix = ".")
