...
gs <- c(1, 2, 3, 4, 5, 6, 7, 8)
num_pops <- c(40)
num_oscs <- c(2, 4, 6, 8, 10, 12)
qs <- c("0.5")

exp_path <- paste(path, "/exp16/", sep="")
mi_exp16 <- load.data7(exp_path, "MI", stim_rates, qs, num_pops, num_oscs, qs)
dmi_exp16 <- load.data7(exp_path, "dMI", stim_rates, qs, num_pops, num_oscs, qs)
h_exp16 <- load.data7(exp_path, "H", stim_rates, qs, num_pops, num_oscs, qs)
pac_exp16 <- load.data7(exp_path, "PAC", stim_rates, qs, num_pops, num_oscs, qs)
pow_exp16 <- load.data7(exp_path, "power", stim_rates, qs, num_pops, num_oscs, qs)
```
...
# Figure - Overall 4 - osc firing rate
...
# Figure - Control
## Other params
```{r, fig.width=2.1, fig.height=2.8, echo=FALSE, warning=FALSE, message=FALSE}
# --------------------------------------------------------------
dmi_exp9 %>% 
  gather(model, mi, back_p:div_sub_p) %>% 
  filter(model %in% c("add_p","mult_p", "sub_p", "div_sub_p")) -> 
