...
exp1path <- paste(path, "/exp7/", sep="")
mi_exp7 <- load.data1(exp1path, "MI", stim_rates, qs, num_pops, qs)
dmi_exp7 <- load.data1(exp1path, "dMI", stim_rates, qs, num_pops, qs)
h_exp7 <- load.data1(exp1path, "H", stim_rates, qs, num_pops, qs)
pac_exp7 <- load.data1(exp1path, "PAC", stim_rates, qs, num_pops, qs)
pow_exp7 <- load.data1(exp1path, "power", stim_rates, qs, num_pops, qs)
```
...
tmp %>% 
  ...
  stat_compare_means(comparisons = my_comparisons, label = "p.signif", label.y = c(0.45, 0.3, 0.15)) +
  theme_classic() -> 
  p1
plot(p1)
```
...
```
# PAC v Power
```{r, fig.width=2.2, fig.height=3.5, echo=FALSE, warning=FALSE, message=FALSE}
# E, I, IE
h_exp7 %>% 
  filter(q == "0.05") %>% 
  filter(num_pop == 50) %>% 
...
tmp %>% 
  filter(stim_rate == 20, model=="EI") %>% 
  ggplot(aes(x=pow, y=pac, color=factor(g))) +
  geom_point(size=1.2, alpha=1) + 
  labs(x="Spectral power", y="PAC") +
  scale_color_manual("g", values = c(
    "lightsteelblue3",
    "lightskyblue3",
    "steelblue3",
...
p1 / p2 / p3 + plot_layout(guides = "collect") +
  plot_annotation(tag_levels = 'a', tag_suffix = ".")
ggsave("figures/pp.pdf", dpi=600)
```
# PAC v Power (bw)
```{r, fig.width=1.4, fig.height=3.5, echo=FALSE, warning=FALSE, message=FALSE}
tmp %>% 
  filter(stim_rate == 20, model=="EI") %>% 
  ggplot(aes(x=pow, y=pac)) +
  geom_point(size=1.2, alpha=1) + 
  labs(x="Spectral power", y="PAC") +
  facet_grid(model~., scale = "free_y") +
  theme_classic2() +
...
p1 / p2 / p3 + plot_layout(guides = "collect") +
  plot_annotation(tag_levels = 'a', tag_suffix = ".")
ggsave("figures/pp_bw.pdf", dpi=600)
```
