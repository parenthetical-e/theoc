...
```{r}
# arrow 
arrow <- data.frame(x=0.5, xend=0.5, y=1, yend=0)
arrow %>% 
...
```{r, fig.width=3, fig.height=4.5, echo=FALSE, warning=FALSE, message=FALSE}
...
examples %>% 
  ...
  theme_classic2() + 
  labs(x="", y="S", tag="a.", title="Timecourse examples") + 
...
ggplot(aes(x=frequency, y=log10(power))) +
  ...
  labs(x="", y="Power", tag="f.", title="Power spectrum") +
  lims(y=c(-3,2)) +
  theme_classic2() +
...
ggplot(aes(x=frequency, y=log10(power))) +
  geom_line(size=0.8, color="black") +
  labs(x="Frequency.", y="Power", tag="k.") +
  lims(y=c(-3,2)) +
  theme_classic2() -> s8


((p4 / p5 / a1 / p6 / p7 / p8) | (s4 / s5 / plot_spacer() / s6 / s7 / s8)) +
  plot_layout(width = c(0.75, 0.25)) +
  # plot_annotation(tag_levels = 'a', tag_suffix = ".")
ggsave("figures/example.pdf", dpi=600, width=2*3, height=2*4.5)
```
...
# Figure - Dists
```{r, fig.width=1.4, fig.height=2.25, echo=FALSE, warning=FALSE, message=FALSE}
example_dists <- read_csv("../data/example_dists.csv")
