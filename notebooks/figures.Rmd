...
examples %>% 
  ...
  scale_y_continuous(breaks=c(0, 0.5, 1), limits=c(0, 1)) +
  theme_classic2() + 
  labs(x="", y="EI", tag="c.",  title="Modulated") +
  theme(axis.title.y=element_text(angle = 0, vjust = 0.5, face = "bold")) + 
  theme(plot.title = element_text(size = 9, hjust=1, face = "bold")) -> 
p6
...
((p4 / p5 / p9 / p6 / p7 / p8) | (s4 / s5 / s9 / s6 / s7 / s8)) +
  plot_layout(width = c(0.75, 0.25)) +
  # plot_annotation(tag_levels = 'a', tag_suffix = ".")
ggsave("figures/example.pdf", dpi=600, width=2*3, height=2*4.8)
```
...
# Figure - Dists
```{r, fig.width=1.4, fig.height=2.5, echo=FALSE, warning=FALSE, message=FALSE}
example_dists <- read_csv("../data/example_dists.csv")
...
example_dists %>% 
  ...
  scale_x_continuous(breaks=0:9, labels=1:10) +
  labs(y="", x="", title="Estimating mutual\ninformation") +
  lims(y=c(0,1)) +
  theme_classic2() + 
  theme(plot.title = element_text(size = 7, hjust=1, face = "bold")) +
