...
# Load data
## Main
```{r load1, message=FALSE, warning=FALSE, error=FALSE, echo=TRUE}
...
examples %>% 
  ...
  scale_y_continuous(breaks=c(0, 0.5, 1), limits=c(0, 1)) +
  theme_classic2() + 
  labs(x="", y="Norm. rate", tag="c.", title="Oscillation (O)") +
  theme(plot.title = element_text(size = 9, face = "bold")) ->
p5
...
ggsave("figures/lnp-example.pdf", dpi=300, width=2*3, height=2*5.4)
ggsave("figures/lnp-example.png", dpi=300, width=2*3, height=2*5.4)
```
...
# Figure - Dists
```{r, fig.width=2.4, fig.height=4.2, echo=FALSE, warning=FALSE, message=FALSE}
example_dists <- read_csv("../data/example_dists.csv")

colors <- colors <- c("Stimulus" = "darkgrey", "Modulated" = "black")

example_dists %>% 
  ggplot(aes(x=m, y=stim_p, fill="Modulated")) +
  geom_bar(stat="identity", alpha=1) +
  geom_bar(aes(x=m, y=stim_ref, fill="Stimulus"), alpha=0.6, stat="identity") +
  scale_x_continuous(breaks=0:9, labels=1:10) +
...
example_dists %>% 
  ggplot(aes(x=m, y=mult_p, fill="Modulated")) +
  geom_bar(stat="identity", alpha=1) +
  geom_bar(aes(x=m, y=stim_ref, fill="Stimulus"),  alpha=0.6, stat="identity") +
  scale_x_continuous(breaks=0:9, labels=1:10) +
  labs(y="", x="", tag="b.") +
...
example_dists %>% 
  ggplot(aes(x=m, y=add_p, fill="Modulated")) +
  geom_bar(stat="identity", alpha=1) +
  geom_bar(aes(x=m, y=stim_ref, fill="Stimulus"), alpha=0.6, stat="identity") +
  scale_x_continuous(breaks=0:9, labels=1:10) +
  labs(y="", x="", tag="c.") +
...
example_dists %>% 
  ggplot(aes(x=m, y=sub_p, fill="Modulated")) +
  geom_bar(aes(x=m, y=stim_ref, fill="Stimulus"), alpha=0.6, stat="identity") +
  geom_bar(stat="identity", alpha=1) +
  scale_x_continuous(breaks=0:9, labels=1:10) +
  labs(y="", x="", tag="d.") +
...
example_dists %>% 
  ggplot(aes(x=m, y=div_sub_p, fill="Modulated")) +
  geom_bar(stat="identity", alpha=1) +
  geom_bar(aes(x=m, y=stim_ref, fill="Stimulus"), alpha=0.6, stat="identity") +
  scale_x_continuous(breaks=0:9, labels=1:10) +
  labs(y="", x="m", tag="e.") +
...
b <- (b0 / b2 / b1 / b3 / b4) 
y_label + b + plot_layout(widths = c(0.1, 0.9), guides = "collect") +
...
p1 + p2 + plot_layout(widths = c(0.15, 0.85)) +
  plot_annotation(tag_levels = 'a', tag_suffix = ".")
ggsave("figures/lnp1.pdf", dpi=300)
ggsave("figures/lnp1.png", dpi=300)
```
...
tmp %>% 
  ...
  labs(y=expression(paste(Delta, "MI (bits)")), x="Oscillation firing rate (spks/s)") +
  scale_color_manual("g", values = c(
    "lightsteelblue3",
    "lightskyblue3",
    "steelblue3",
...
ggsave("figures/lnp2.pdf", dpi=300)
ggsave("figures/lnp2.png", dpi=300)
```
...
((p1 / p3) | (p2 / p4)) + 
  ...
  plot_annotation(tag_levels = 'a', tag_suffix = ".", 
                  ...
                  theme=theme(plot.title = element_text(size = 11, face = "bold")))
ggsave("figures/lnp3.pdf", dpi=300)
ggsave("figures/lnp3.png", dpi=300)
```
...
p1 + p2 + plot_layout(widths = c(0.07, 0.9)) +
  plot_annotation(tag_levels = 'a', tag_suffix = ".", 
                  title="Population size: 40",
                  theme=theme(plot.title = element_text(size = 11, face = "bold")))
ggsave("figures/lnp4.pdf", dpi=300)
ggsave("figures/lnp4.png", dpi=300)
```
...
tmp %>% 
  ...
  labs(y=expression(paste(Delta, "MI (bits)")), x="Oscillation frequency (Hz)") +
  theme_classic() +
  theme(strip.background = element_blank()) +
  theme(strip.text.x = element_text(face = "bold")) +
...
p1 / p4 / p5 + 
  plot_annotation(tag_levels = 'a', tag_suffix = ".",
                  title="Population size: 40", 
                  theme=theme(plot.title = element_text(size = 11, face = "bold")))
ggsave("figures/lnp-control1.pdf", dpi=300)
ggsave("figures/lnp-control1.png", dpi=300)
```
...
p1 / p2 + 
  plot_annotation(tag_levels = 'a', tag_suffix = ".")
ggsave("figures/lnp-power1.pdf", dpi=300)
ggsave("figures/lnp-power1.png", dpi=300)
```
...
ggsave("figures/lnp-power2.pdf", dpi=300)
ggsave("figures/lnp-power2.png", dpi=300)
```
