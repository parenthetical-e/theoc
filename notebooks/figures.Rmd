...
```{r, fig.width=1.1, fig.height=3.1, echo=FALSE, warning=FALSE, message=FALSE}
h_exp1 %>% 
  filter(q == "0.0") %>% 
  filter(num_pop == 50) %>% 
...
tmp %>% 
  ...
  theme(strip.text.y = element_text(angle = 0, face = "bold")) +
  # stat_cor(method = "spearman", label.x = 0.01, label.y = 0.72, cor.coef.name = "rho") +
  facet_grid(model~.) ->
  p2
# plot(p2)
